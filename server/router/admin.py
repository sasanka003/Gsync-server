...
def edit_gardener(user_id:uuid.UUID, request:EditGardener, db: Session = Depends(get_db), token: dict = Depends(admin_only)): #token: dict = Depends(get_current_user)
...
def get_all_plantations(db: Session = Depends(get_db), token: dict = Depends(admin_only)): #token: dict = Depends(get_current_user)
