...
@router.get("/get", description='get all plantations of a user', response_description="all plantations of a user", response_model=PlantationDisplay, responses={404: {"description": "Plantations not found"}})
def get_user_plantations(db: Session = Depends(get_db), token: dict = Depends(get_current_user)):   
    plantations = db_plantation.get_user_plantations(db, token)
    if plantations:
